...
DepthSensorDevice::DepthSensorDevice() {
  ...
  MYLOG_INFO("[Device:{}] 构造完成 (DepthSensorDevice)", device_id_);
}

DepthSensorDevice::DepthSensorDevice(const nlohmann::json& cfg, std::string* err) : DepthSensorDevice() {
  MYLOG_INFO("[Device:{}] 通过配置构造 DepthSensorDevice，cfg={}", device_id_, cfg.dump());
  try {
    if (!Init(cfg, err)) {
...
bool DepthSensorDevice::Init(const nlohmann::json& cfg, std::string* err) {
  ...
  try {
    ...
    control_ = my_control::MyControl::GetInstance().CreateControl("depth_sensor");
    if (!control_) {
      if (err) *err = "CreateControl(depth_sensor) failed";
      MYLOG_ERROR("[Device:{}] Init 失败：CreateControl(depth_sensor) 返回 nullptr", device_id_);
      return false;
    }
...
bool DepthSensorDevice::Start(my_control::TaskQueue& queue, std::atomic<bool>* estop_flag, std::string* err) {
...
my_data::DeviceStatus DepthSensorDevice::GetStatusSnapshot() const {
  std::lock_guard<std::mutex> lk(status_mu_);
  return status_;
}

void DepthSensorDevice::UpdateOnTaskStart(const my_data::Task& task) {
  std::lock_guard<std::mutex> lk(status_mu_);
...
void DepthSensorDevice::UpdateOnTaskFinish(const my_data::Task& task, const my_data::TaskResult& r) {
  std::lock_guard<std::mutex> lk(status_mu_);
