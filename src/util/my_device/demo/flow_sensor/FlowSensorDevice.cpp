...
FlowSensorDevice::FlowSensorDevice() {
  ...
  MYLOG_INFO("[Device:{}] 构造完成 (FlowSensorDevice)", device_id_);
}
...
bool FlowSensorDevice::Init(const nlohmann::json& cfg, std::string* err) {
  ...
  try {
    ...
    control_ = my_control::MyControl::GetInstance().CreateControl("flow_sensor");
    if (!control_) {
...
my_data::DeviceStatus FlowSensorDevice::GetStatusSnapshot() const {
  std::lock_guard<std::mutex> lk(status_mu_);
  return status_;
}

void FlowSensorDevice::UpdateOnTaskStart(const my_data::Task& task) {
  std::lock_guard<std::mutex> lk(status_mu_);
...
void FlowSensorDevice::UpdateOnTaskFinish(const my_data::Task& task, const my_data::TaskResult& r) {
  std::lock_guard<std::mutex> lk(status_mu_);
