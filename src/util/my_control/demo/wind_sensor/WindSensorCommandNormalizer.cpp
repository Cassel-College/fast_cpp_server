#include "WindSensorCommandNormalizer.h"
...
std::optional<my_data::Task> WindSensorCommandNormalizer::Normalize(const my_data::RawCommand& in,
                                                                          const my_data::EdgeId& edge_id,
                                                                          std::string* err) {
  MYLOG_INFO("[Normalizer:SplitSpeedSensor] Normalize begin: command_id={}, source={}, received_at_ms={}",
             in.command_id, in.source, in.received_at_ms);

  if (!in.payload.is_object()) {
    if (err) *err = "payload is not a json object";
    MYLOG_ERROR("[Normalizer:SplitSpeedSensor] payload not object. raw={}", in.toString());
    return std::nullopt;
  }
  ...
  if (device_id.empty()) {
    ...
    MYLOG_ERROR("[Normalizer:SplitSpeedSensor] missing device_id. payload={}", p.dump());
    return std::nullopt;
  }
  ...
  MYLOG_INFO("[Normalizer:SplitSpeedSensor] Normalize ok: {}", task.toString());
  return task;
}
